...
from config import DATA_DIR, GAMES_JSON
...
def fetch_top_games(n=200):
    ...
    if not csv_path.exists():
        raise FileNotFoundError(
            f"Ficheiro {csv_path} não encontrado.\n"
                ...
                games.append(
                    {
                        ...
                        "year": int(
                            row.get("YearPublished") or row.get("year_published") or 0
                        ),
                        "min_players": int(
                            row.get("MinPlayers") or row.get("min_players") or 1
                        ),
                        "max_players": int(
                            row.get("MaxPlayers") or row.get("max_players") or 4
                        ),
                        "playtime": int(
                            row.get("MfgPlaytime") or row.get("playtime") or 60
                        ),
                        "min_age": int(row.get("MfgAgeRec") or row.get("min_age") or 10),
                        "weight": float(
                            row.get("ComAgeRec") or row.get("weight") or 2.0
                        ),
                        "rating": rating,
                        "rank": str(
                            row.get("Rank:boardgame") or row.get("rank") or i + 1
                        ),
                        "categories": [
                            c.strip()
                            for c in (row.get("GameType") or "").split(",")
                            if c.strip()
                        ],
                        "mechanics": [
                            m.strip()
                            for m in (row.get("Mechanics") or "").split(",")
                            if m.strip()
                        ],
    ...
    print(f"Guardados {len(games)} jogos em {GAMES_JSON}")
    return games
